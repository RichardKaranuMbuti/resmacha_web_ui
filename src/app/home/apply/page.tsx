...
const ApplyPageContent = () => {
  const { initiateScraping, refreshData } = useScrapingContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await initiateScraping(jobTitle.trim(), location.trim());
      
      // Show loading animation for a brief moment before context takes over
      setTimeout(() => {
        setIsLoading(false);
        // Clear form
        setJobTitle('');
        setLocation('');
        // Refresh data to trigger the status guard
        refreshData();
      }, 2000);
